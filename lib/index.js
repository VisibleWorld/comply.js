...
function testNonSchemaValidators(rules, o, extra, k) {
    var run = _.curry(runValidator)(o)(k)(extra),
        ...
        presence = k in o,
        pass = (!required || presence) && _(rules[k].validators).reject(isSchema).map(run).every();

    return validationResult(pass, (required && !presence) ? '"' + k + '" is required.' : rules[k].message, k === '*' ? '' : k, function() {
